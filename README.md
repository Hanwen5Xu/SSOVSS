# A text-supervised open-vocabulary semantic segmentation method with spatial-semantic feature fusion
This repository is the official implementation of "A coarse-to-fine framework for open-vocabulary semantic segmentation of remote sensing images".
...
1. Using [Janus](https://github.com/deepseek-ai/Janus), [Qwen3-VL](https://huggingface.co/Qwen/Qwen3-VL-8B-Instruct) or other MLLMS to generate textual descriptions for remote sensing images. We provide pre-generated textual descriptions of 20 words and 50 words in length, as given in [train_caption_20words.csv](https://drive.google.com/file/d/1RbSsL9_7HPPHkt_px1pYoBM9XfYwJUHV/view?usp=drive_link) and [train_caption_50words.csv](https://drive.google.com/file/d/1RbSsL9_7HPPHkt_px1pYoBM9XfYwJUHV/view?usp=drive_link).
2. Using large language models such as DeepSeek, Gemini, and GPT to extract remote sensing object entities from the textual descriptions, as shown in [geo_entities.py](data_tools/geo_entities.py).
...
```
6. Modify the img_dir and metas_path in [config_coarse.yaml](configs/config_coarse.yaml).
...
```
2. Change the configs[config_coarse.yaml](configs/config_coarse.yaml) by specifying the checkpoint path.
```shell
dino_path: '/path/to/your/dino_vitbase16_pretrain.pth'
```

## Coarse-level training
Using [main_coarse.py](main_coarse.py) to perform text-supervised contrastive training, and the trained coarse-grained model is saved to checkpoint. We have provided a pre-trained coarse-grained model, [net_GID.pth](https://drive.google.com/file/d/1L9gXQ2sZ_A0z8240g5VIEJjJ8fZJ1v1A/view?usp=drive_link), which was trained on the GID dataset.

## Fine-level inference
Using [main_infer.py](main_infer.py) to perform fine-level open-vocabulary semantic segmentation. Users can input custom text vocabularies to execute the inference.
