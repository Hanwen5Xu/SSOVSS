...
from data_tools.geo_entities import Entities_GID_15, Entities_GID_50
...
for key in Entities_GID_15.keys():
    TOP_CLASSES += Entities_GID_15[key]


def judge_noun(word):
    if word in TOP_CLASSES:
        return 1
    return 0
...
def make_filter(infos):
    ...
    for i, item in enumerate(subset_list.iterrows()):
        each_cap = item[1]['caption'].lower()

        valid_words = []
        for word in TOP_CLASSES:
            word = word.lower()
            word = to_singular(word)
            word_s = to_plural(word)

            if word in each_cap or word_s in each_cap:
    ...
    new_dataframe.to_csv(f'{args.dstdir}/gid_{cur_index}.csv', index=False)
    return
...
def merge_all_subset(args):
    ...
    all_files = sorted(all_files, key=extract_num)
    ...
    all_data.to_csv(os.path.join(args.dstdir, 'gid_filtered_subset.csv'), index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--processors', type=int, default=32, help='processors for data filtering')
    ...
    parser.add_argument('--metafile', type=str, default=None,
                        help='the metafile used for constructing cross-image pairs')
